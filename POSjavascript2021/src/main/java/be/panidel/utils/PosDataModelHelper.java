...
public class PosDataModelHelper {
	...
	public static RootPos10 generatePos10Model(String fileName, ProductModels productModels)
		...
		for (ProductModel product : productModels.getProducts()) {
			...
			if (product.getMini() != null && product.getMini() > 0) {
			...
			if (product.getNormal() != null && product.getNormal() > 0) {
				prix = product.getNormal();
				code = product.getCode();
				ProductPos10 product10 = implementProduct(product, prix, code);
				productsPos10.getProductPos10s().add(product10);
			}
			if (product.getGeant() != null && product.getGeant() > 0) {
			...
			if (product.getFitmini() != null && product.getFitmini() > 0) {
				prix = product.getFitmini();
				code = "M" + product.getCode() + "FIT";
				ProductPos10 product10 = implementProduct(product, prix, code);
				productsPos10.getProductPos10s().add(product10);
			}
			if (product.getFitnormal() != null && product.getFitnormal() > 0) {
				prix = product.getFitnormal();
				code = product.getCode() + "FIT";
				ProductPos10 product10 = implementProduct(product, prix, code);
				productsPos10.getProductPos10s().add(product10);
			}
			if (product.getFitgeant() != null && product.getFitgeant() > 0) {
	...
	private static ProductPos10 implementProduct(ProductModel product, Integer prix, String code) {
		...
		ProductPos10 product10 = new ProductPos10(id, code, nom, description, htmlKeyLabel, prixachat, prix,
